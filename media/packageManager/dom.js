/* Package Manager Webview DOM */
(function () {
    window.PM = window.PM || {};

    window.PM.dom = {
        btnCreate: document.getElementById('btnCreate'),
        btnRefresh: document.getElementById('btnRefresh'),
        btnShowEnvironment: document.getElementById('btnShowEnvironment'),
        btnOpenBuildCheckSettings: document.getElementById('btnOpenBuildCheckSettings'),
        btnNewRosTerminal: document.getElementById('btnNewRosTerminal'),
        btnToggleTerminalList: document.getElementById('btnToggleTerminalList'),
        btnOpenCreate: document.getElementById('btnOpenCreate'),
        btnCloseCreate: document.getElementById('btnCloseCreate'),
        btnCancelCreate: document.getElementById('btnCancelCreate'),
        createModal: document.getElementById('createModal'),
        createBackdrop: document.getElementById('createBackdrop'),
        pkgNameInput: document.getElementById('pkgName'),
        pkgNameNormalizedHint: document.getElementById('pkgNameNormalizedHint'),
        buildTypeInput: document.getElementById('buildType'),
        licenseInput: document.getElementById('pkgLicense'),
        descriptionInput: document.getElementById('pkgDescription'),
        depsInput: document.getElementById('deps'),
        statusEl: document.getElementById('createStatus'),
        pkgStatusEl: document.getElementById('pkgStatus'),
        filterInput: document.getElementById('pkgFilter'),
        pkgCount: document.getElementById('pkgCount'),
        btnToggleWorkspacePackages: document.getElementById('btnToggleWorkspacePackages'),
        lblToggleWorkspacePackages: document.getElementById('lblToggleWorkspacePackages'),
        btnToggleOtherPackages: document.getElementById('btnToggleOtherPackages'),
        lblToggleOtherPackages: document.getElementById('lblToggleOtherPackages'),
        lblToggleTerminalList: document.getElementById('lblToggleTerminalList'),
        btnLoadOtherPackages: document.getElementById('btnLoadOtherPackages'),
        otherPkgList: document.getElementById('otherPkgList'),
        pinnedList: document.getElementById('pinnedList'),
        terminalList: document.getElementById('terminalList'),
        argsModal: document.getElementById('argsModal'),
        argsBackdrop: document.getElementById('argsBackdrop'),
        configName: document.getElementById('configName'),
        configRunTarget: document.getElementById('configRunTarget'),
        configRunTargetDescription: document.getElementById('configRunTargetDescription'),
        configList: document.getElementById('configList'),
        btnAddConfig: document.getElementById('btnAddConfig'),
        btnRemoveConfig: document.getElementById('btnRemoveConfig'),
        selectedArgsList: document.getElementById('selectedArgsList'),
        btnAddCustomArg: document.getElementById('btnAddCustomArg'),
        argsList: document.getElementById('argsList'),
        btnInsertAll: document.getElementById('btnInsertAll'),
        btnSaveArgs: document.getElementById('btnSaveArgs'),
        btnCancelArgs: document.getElementById('btnCancelArgs'),
        btnCloseArgs: document.getElementById('btnCloseArgs'),
        buildCheckSettingsModal: document.getElementById('buildCheckSettingsModal'),
        buildCheckSettingsBackdrop: document.getElementById('buildCheckSettingsBackdrop'),
        buildCheckScopeSettings: document.getElementById('buildCheckScopeSettings'),
        btnCloseBuildCheckSettings: document.getElementById('btnCloseBuildCheckSettings'),
        btnCancelBuildCheckSettings: document.getElementById('btnCancelBuildCheckSettings'),
        btnSaveBuildCheckSettings: document.getElementById('btnSaveBuildCheckSettings'),
        toggleBuildCheck: document.getElementById('toggleBuildCheck'),
        toggleBuildCheckLaunchFiles: document.getElementById('toggleBuildCheckLaunchFiles'),
        toggleBuildCheckRunNodes: document.getElementById('toggleBuildCheckRunNodes'),
        addNodeModal: document.getElementById('addNodeModal'),
        addNodeBackdrop: document.getElementById('addNodeBackdrop'),
        addNodePkg: document.getElementById('addNodePkg'),
        addNodeName: document.getElementById('addNodeName'),
        addNodeNameNormalizedHint: document.getElementById('addNodeNameNormalizedHint'),
        addNodeTemplate: document.getElementById('addNodeTemplate'),
        addNodeTopicRow: document.getElementById('addNodeTopicRow'),
        addNodeTopic: document.getElementById('addNodeTopic'),
        addNodeTopicNormalizedHint: document.getElementById('addNodeTopicNormalizedHint'),
        btnAddNode: document.getElementById('btnAddNode'),
        btnCancelAddNode: document.getElementById('btnCancelAddNode'),
        btnCloseAddNode: document.getElementById('btnCloseAddNode'),
        addNodeStatus: document.getElementById('addNodeStatus'),
        createLaunchModal: document.getElementById('createLaunchModal'),
        createLaunchBackdrop: document.getElementById('createLaunchBackdrop'),
        createLaunchPkg: document.getElementById('createLaunchPkg'),
        createLaunchName: document.getElementById('createLaunchName'),
        createLaunchNameNormalizedHint: document.getElementById('createLaunchNameNormalizedHint'),
        btnCreateLaunch: document.getElementById('btnCreateLaunch'),
        btnCancelCreateLaunch: document.getElementById('btnCancelCreateLaunch'),
        btnCloseCreateLaunch: document.getElementById('btnCloseCreateLaunch'),
        createLaunchStatus: document.getElementById('createLaunchStatus'),
        removeLaunchModal: document.getElementById('removeLaunchModal'),
        removeLaunchBackdrop: document.getElementById('removeLaunchBackdrop'),
        removeLaunchPkg: document.getElementById('removeLaunchPkg'),
        removeLaunchName: document.getElementById('removeLaunchName'),
        removeLaunchPath: document.getElementById('removeLaunchPath'),
        btnRemoveLaunch: document.getElementById('btnRemoveLaunch'),
        btnCancelRemoveLaunch: document.getElementById('btnCancelRemoveLaunch'),
        btnCloseRemoveLaunch: document.getElementById('btnCloseRemoveLaunch'),
        removeLaunchStatus: document.getElementById('removeLaunchStatus'),
        removeNodeModal: document.getElementById('removeNodeModal'),
        removeNodeBackdrop: document.getElementById('removeNodeBackdrop'),
        removeNodePkg: document.getElementById('removeNodePkg'),
        removeNodeName: document.getElementById('removeNodeName'),
        removeNodePath: document.getElementById('removeNodePath'),
        btnRemoveNode: document.getElementById('btnRemoveNode'),
        btnCancelRemoveNode: document.getElementById('btnCancelRemoveNode'),
        btnCloseRemoveNode: document.getElementById('btnCloseRemoveNode'),
        removeNodeStatus: document.getElementById('removeNodeStatus'),
        environmentModal: document.getElementById('environmentModal'),
        environmentBackdrop: document.getElementById('environmentBackdrop'),
        environmentLoadingState: document.getElementById('environmentLoadingState'),
        environmentDetailsToggle: document.getElementById('environmentDetailsToggle'),
        environmentDetailsContainer: document.getElementById('environmentDetailsContainer'),
        environmentDetails: document.getElementById('environmentDetails'),
        runEnvironmentTarget: document.getElementById('runEnvironmentTarget'),
        runEnvironmentDescription: document.getElementById('runEnvironmentDescription'),
        runTerminalMode: document.getElementById('runTerminalMode'),
        runTerminalModeDescription: document.getElementById('runTerminalModeDescription'),
        btnCloseEnvironment: document.getElementById('btnCloseEnvironment'),
        btnCancelEnvironment: document.getElementById('btnCancelEnvironment'),
        btnSaveEnvironment: document.getElementById('btnSaveEnvironment'),
    };
})();
